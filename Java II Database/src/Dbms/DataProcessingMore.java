...
public class DataProcessingMore {
	...
	public static void extraordinaryOption(ArrayList<ArrayList<Object>> lists, String mathOperation) {
		...
		for (int i = 0; i < mathOperation.length(); i++) {
			if (Character.isLetter(mathOperation.charAt(i))) {
				...
				while (Character.isDigit(mathOperation.charAt(j)) || mathOperation.charAt(j) == '.') {
		...
		while (countHasStopped < position.size()) {
			if (Character.isLetter(mathOperation.charAt(position.get(countHasStopped)))) {
				int at = whereIsField(lists, fields[countFields]);
				if ((at > -1 && at < lists.size()) && !wasInsideOnce) {
					wasInsideOnce = true;
					for (int a = 1; a < lists.get(whereIsField(lists, fields[countFields])).size(); a++) {
			...
			} else if (Character.isDigit(mathOperation.charAt(position.get(countHasStopped)))) {
				if (!wasInsideOnce) {
					wasInsideOnce = true;
					...
					switch (mathOperation.charAt(position.get(countHasStopped - 1))) { // to aaaaaaaaaaiiiiiiiiiiiiiiiiiiiiiiiiiiiii
					case '+':
						for (int b = 1; b < lists.get(lists.size() - 1).size(); b++) {
							double temp1 = Double.parseDouble((String) lists.get(lists.size() - 1).get(b));
	...
	public static int whereIsField(ArrayList<ArrayList<Object>> lists, String given) {
		int i = 0;
	...
	public static boolean areAllNumbers(ArrayList<ArrayList<Object>> lists, int askedColumn) {
		boolean areAllNumbers = true;
		for (int i = 1; i < lists.get(askedColumn).size(); i++) {
			if (DatabaseProcessing.isNumber((String) lists.get(askedColumn).get(i)) == false) {
